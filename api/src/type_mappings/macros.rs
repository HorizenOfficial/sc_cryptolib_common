macro_rules! generate_algebraic_types {
    ($curve: ident, $curve_parameters: ty) => {
        ...
        pub const FIELD_SIZE: usize = ((
            <FieldElement as PrimeField>::Params::MODULUS_BITS +
        ...
        pub const GROUP_COMPRESSED_SIZE: usize = FIELD_SIZE + if <FieldElement as PrimeField>::Params::REPR_SHAVE_BITS >= 2 { 0 } else { 1 };
    }
}
...
macro_rules! generate_schnorr_signature_types {
    ($projective_curve: ident, $affine_curve: ident) => {
        ...
        pub type SchnorrSigScheme = FieldBasedSchnorrSignatureScheme<FieldElement, $projective_curve, FieldHash>;
