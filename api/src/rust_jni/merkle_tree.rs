...
) -> jboolean
{
    ...
    let root = parse_rust_struct_from_jobject::<FieldElement>(&_env, _root, "fieldElementPointer");

    let path = parse_rust_struct_from_jobject::<GingerMHTPath>(&_env, _path, "merklePathPointer");
...
{
    drop_rust_struct_from_jobject::<GingerMHT>(_env, _tree, "inMemoryOptimizedMerkleTreePointer")
});