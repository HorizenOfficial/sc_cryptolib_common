...
sed -i 's!<artifactId>sc-common-cryptolib</artifactId>!<artifactId>sc-common-cryptolib-tweedle</artifactId>!g' jni/pom.xml

set -xeo pipefail

retval=0

cargo $CARGOARGS build --release --features "tweedle" --target=x86_64-pc-windows-gnu || retval="$?"
cargo $CARGOARGS build --release --features "tweedle" --target=x86_64-unknown-linux-gnu || retval="$?"
cargo $CARGOARGS build --features "tweedle" --tests || retval="$?"
